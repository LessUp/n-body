...
This path keeps the build configuration aligned with the repository scripts and generates the compile database used by the recommended editor/LSP setup.
...
| `NBODY_BUILD_EXAMPLES` | `ON` | Build example executables |
...
1. [Architecture]({% link docs/architecture.md %}) - Understand system design
2. [Algorithms]({% link docs/algorithms.md %}) - Learn force calculation methods
3. [API Reference]({% link docs/api.md %}) - Use the library programmatically
4. [Performance Guide]({% link docs/performance.md %}) - Optimize for your use case
