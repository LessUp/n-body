...
__global__ void copyPositionsToVBOKernel(float *vbo, const float *pos_x,
                                         const float *pos_y, const float *pos_z,
                                         int N) {
...
void launchCopyPositionsToVBOKernel(float *d_vbo,
...
CudaGLInterop::CudaGLInterop()
    : position_vbo_(0), cuda_vbo_(nullptr), is_mapped_(false),
      is_initialized_(false), particle_count_(0) {}

CudaGLInterop::~CudaGLInterop() { cleanup(); }
...
void CudaGLInterop::initialize(size_t particle_count) {
  ...
  glBufferData(GL_ARRAY_BUFFER, particle_count * 3 * sizeof(float), nullptr,
               GL_DYNAMIC_DRAW);
...
bool CudaGLInterop::verifyDataIntegrity(const float *expected_data,
                                        size_t count) {
