...
public:
  ...
  void computeForces(ParticleData *d_particles, float cutoff, float G,
                     float eps);
  ...
  void copyCellDataToHost(std::vector<int> &cell_start,
  ...
  __host__ __device__ static int3 getCellIndex(float x, float y, float z,
                                               float cell_size);
...
void launchAssignCellsKernel(const ParticleData *d_particles,
                             int *d_particle_cell, float cell_size,
...
void launchPrefixSumKernel(int *d_cell_counts, int *d_cell_start,
                           int total_cells);
void launchSortParticlesByCellKernel(const int *d_particle_cell,
                                     const int *d_cell_start,
                                     int *d_sorted_indices,
                                     int *d_cell_counts_temp,
                                     int particle_count);
void launchComputeCellEndKernel(const int *d_cell_start, int *d_cell_end,
                                const int *d_cell_counts, int total_cells);
void launchSpatialHashForceKernel(
    const int *d_cell_start, const int *d_cell_end, const int *d_sorted_indices,
    const ParticleData *d_particles, float *d_acc_x, float *d_acc_y,
    float *d_acc_z, int3 grid_dims, float cell_size, const Vec3 &bbox_min,
