...
public:
  ...
  void computeForces(ParticleData *d_particles, float theta, float G,
                     float eps);
...
void launchComputeBoundingBoxKernel(const ParticleData *d_particles,
                                    Vec3 *d_min, Vec3 *d_max);
void launchComputeMortonCodesKernel(const ParticleData *d_particles,
                                    unsigned int *d_codes, const Vec3 &bbox_min,
                                    const Vec3 &bbox_max);
...
void launchBarnesHutForceKernel(const OctreeNode *d_nodes,
                                const ParticleData *d_particles, float *d_acc_x,
                                float *d_acc_y, float *d_acc_z, int N,
                                float theta, float G, float eps2,
                                int block_size);
